...
@property (nonatomic,strong) UILabel  *trackPoint;//小红点
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self.trackPoint = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 10, 10)];
        self.trackPoint.layer.masksToBounds = YES;
        self.trackPoint.layer.cornerRadius = 2.5;
        self.trackPoint.backgroundColor = [UIColor redColor];
        [self addSubview:self.trackPoint];
        [self.trackPoint mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)internalMethod_changeSelectedButton:(UIButton *)button {
    if (self.tempBtn == nil) {
        button.selected = YES;
        self.tempBtn = button;
        button.backgroundColor = self.titleBackColorStateSelected;
        button.titleLabel.font = self.titleFontStateSelected;
        ...
        self.tempBtn.backgroundColor = self.titleBackColorStateNormal;
        self.tempBtn.titleLabel.font = self.titleFontStateNormal;
        button.selected = YES;
        button.backgroundColor = self.titleBackColorStateSelected;
        button.titleLabel.font = self.titleFontStateSelected;
        self.tempBtn = button;
...
- (void)internalMethod_changeIndicatorViewLocationWithButton:(UIButton *)button {
    if (self.allBtnWidth <= self.bounds.size.width) { /// SGPageTitleView 不可滚动
        [UIView animateWithDuration:_indicatorAnimationTime animations:^{
            ...
            } else if (self.indicatorLengthStyle == SGIndicatorLengthStyleFixed) {
                self.indicatorView.SG_width = self.indicatorWidth;
            } else {
...
- (void)setTitleFontStateNormal:(UIFont *)titleFontStateNormal{
    _titleFontStateNormal = titleFontStateNormal;
    [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIButton *btn = obj;
        if (!btn.selected) {
            btn.titleLabel.font = titleFontStateNormal;
        }
    }];
    //    [self setupStartColor:titleColorStateNormal];
}

- (void)setTitleFontStateSelected:(UIFont *)titleFontStateSelected{
    _titleFontStateSelected = titleFontStateSelected;
    [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIButton *btn = obj;
        if (btn.selected) {
            btn.titleLabel.font = titleFontStateSelected;
        }
...
//-(void)setTitleBackColorStateNormal:(UIColor *)titleBackColorStateNormal{
//
//    _titleBackColorStateNormal = titleBackColorStateNormal;
////    [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
////        UIButton *btn = obj;
////        btn.backgroundColor = titleBackColorStateNormal;
////    }];
////    [self setupStartColor:titleBackColorStateNormal];
//}

//- (void)setTitleBackColorStateSelected:(UIColor *)titleBackColorStateSelected{
//    _titleBackColorStateSelected = titleBackColorStateSelected;
//    [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        UIButton *btn = obj;
//        btn.backgroundColor = titleBackColorStateSelected;
//    }];
////    [self setupEndColor:titleBackColorStateSelected];
//}
...
- (void)setIndicatorWidth:(CGFloat)indicatorWidth{
    ...
    if (indicatorWidth) {
        _indicatorWidth = indicatorWidth;
        self.indicatorView.SG_width = indicatorWidth;
        self.indicatorView.SG_x = self.SG_width - indicatorWidth;
    }
}
...
- (void)setIndicatorLengthStyle:(SGIndicatorLengthStyle)indicatorLengthStyle {
    ...
    } else if (indicatorLengthStyle == SGIndicatorLengthStyleFixed) {
        indicatorViewW = self.indicatorWidth;
        indicatorViewX = 0.5 * (firstBtn.SG_width - self.indicatorWidth);
    } else {
...
-(void)setIsHiddenAllTrackPoint:(BOOL)isHiddenAllTrackPoint{
    
    for (int i = 0; i < self.btnMArr.count; i++) {
        SGPageTitleButton *button = (SGPageTitleButton *)self.btnMArr[i];
        button.trackPoint.hidden = isHiddenAllTrackPoint;
    }
}

-(void)showTrackPoint:(NSInteger)indexPonit hiddenTrackPoint:(BOOL)hidden{
    
    SGPageTitleButton *button = (SGPageTitleButton *)self.btnMArr[indexPonit];
    button.trackPoint.hidden = hidden;
}
